...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (window->DC.LastItemStatusFlags & ImGuiItemFlags_ButtonRepeat)
...
bool ImGui::ButtonExx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
     struct animt
    {
        int active_outline_anim;
    ...
    static std::map <ImGuiID, animt> anim;
    auto it_anim = anim.find(id);
    if (it_anim == anim.end())
    {
        anim.insert({ id, {0, 0, 100} });
        it_anim = anim.find(id);
    }

    if (v)
        it_anim->second.active_outline_anim += 2 * (1.f - ImGui::GetIO().DeltaTime);
    else
        it_anim->second.active_outline_anim -= 2 * (1.f - ImGui::GetIO().DeltaTime);
    ...
    if (v)
        it_anim->second.active_text_anim += 5 * (1.f - ImGui::GetIO().DeltaTime);
    else
        it_anim->second.active_text_anim -= 5 * (1.f - ImGui::GetIO().DeltaTime);
    ...
    if (v)
        it_anim->second.active_rect_alpha += 15 * (0.5f - ImGui::GetIO().DeltaTime);
    else
        it_anim->second.active_rect_alpha -= 15 * (0.5f - ImGui::GetIO().DeltaTime);

    if (it_anim->second.active_rect_alpha > 155)
        it_anim->second.active_rect_alpha = 155;
    else if (it_anim->second.active_rect_alpha < 0)
        it_anim->second.active_rect_alpha = 0;


    window->DrawList->AddRectFilled(rect.Min, rect.Max, ImColor(55, 55, 55, 255), 3.f);

    window->DrawList->AddRectFilled(rect.Min, rect.Max, ImColor(77, 77, 77, it_anim->second.active_rect_alpha), 3.f);

    window->DrawList->AddRect(rect.Min, rect.Max, ImColor(255, 255, 255, (15 + it_anim->second.active_outline_anim)), 3.f);

    PushStyleColor(ImGuiCol_Text, ImVec4(ImColor(255, 255, 255, 100 + it_anim->second.active_rect_alpha)));
    RenderTextClipped(rect.Min + ImVec2(0, 1), rect.Max + ImVec2(0, 1), label, NULL, &label_size, ImVec2(0.5f, 0.5f));
    PopStyleColor();
...
{
    return ButtonExx(label, size_arg, ImGuiButtonFlags_None);
}
...
bool ImGui::InputText2(const char* label, char* buf, size_t buf_size, ImVec2 Size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
    return InputTextEx(label, NULL, buf, (int)buf_size, ImVec2(Size.x, Size.y), flags, callback, user_data);
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (!is_multiline)
    {
        ...
        draw_window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(240, 240, 240, 10), 5);

        draw_window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(240, 240, 240, it_anim->second.active_rect_alpha),5);
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRect(rect.Min, rect.Max, bg_color);
